...
# ModelFusion runtime v2.6 (LIVE MODE + Priority Fix + Warmup temp rule)
# CHANGE (v2.6): ForcedWarmup no longer hardcodes "Engine Off (cold)".
# It uses the temperature rule (cold < 50, warm 50-120, generic otherwise),
# IDENTICAL to the benchmark labeler, so the two stay matched.
...
def _off_label_from_temp(temp: float) -> str:
    """Same temperature rule as the benchmark labeler (cold < 50,
    warm 50-120, generic otherwise). Keep IDENTICAL in both files."""
    if np.isfinite(temp) and (-20.0 <= temp <= 49.999999):
        return "Engine Off (cold)"
    if np.isfinite(temp) and (50.0 <= temp <= 120.0):
...
class ModelFusion:
    ...
    def step_with_sample(self, sample: np.ndarray) -> Dict[str, Any]:
        ...
        # Forced warmup (first 4 samples; label follows temperature rule)
        if self.warmup_counter < HISTORY_SECONDS:
            self.warmup_counter += 1
            return {
                "route": "ForcedWarmup",
                "final": _off_label_from_temp(s_num[0]),   # CHANGED: was hardcoded cold
